...
| `service_running` | ob der Service tatsächlich **läuft** (nicht nur, ob das Control-Binary da ist). Service-Name aus dem letzten Token von `service.stop.<os>`, Status via `launchctl list` / `systemctl is-active` / `sc query`. Windows: gibt `sc query` Fehler 1060 ("kein Dienst"), wird der Name als **Anzeigename** behandelt und per `sc getkeyname` zum Key-Namen aufgelöst (denn `net` akzeptiert Anzeigenamen, `sc query` nur den Key-Namen); der Lauf-Status wird über den **numerischen** State-Code (locale-fest, deutsches Windows lokalisiert die State-Wörter) erkannt. Skipped mit `--skip-service`. Befund nicht eindeutig (unbekannter Manager, Status-Tool fehlt, Output nicht parsebar, Timeout) ⇒ `ok:true` | nein |
